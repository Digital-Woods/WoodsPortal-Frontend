const EmptyMessageCard = ({
  ...
  const [RandomComponent, setRandomComponent] = useState(
    <EmptyDeal width="150" height="130" key="deal" />
  );
  ...
  return (
    <div
      className={`mt-4 w-fit max-w-[600px] mx-auto min-h-48 flex ${
        type === "row" ? "max-sm:flex-col" : "flex-col"
      } items-center justify-center h-auto text-center md:gap-8 gap-4 dark:bg-dark-300 rounded-md ${className} `}
    >
      {/* Illustration */}
      ...
      <div className="flex flex-col gap-2">
        ...
        >
          It looks like you haven’t created any {name.toLowerCase()} yet. Please
          create one to proceed. <br /> If you need assistance with setting up
          or understanding {name.toLowerCase()}, refer to our documentation or
          contact support.
        </p>
        {/* Bullet Points */}
        {bulletPoints.length > 0 && (
          ...
          >
            {bulletPoints.map((point, index) => (
              <li
                key={index}
                className={type === "row" ? "text-start" : "text-center"}
              >
                {point}
              </li>
            ))}
