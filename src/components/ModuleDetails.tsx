import { hubSpotTableData } from "@/data/hubSpotData";
import { sortData, getFileDetails } from "@/utils/DataMigration";
import { env } from "@/env";
import { useState, useEffect } from "react";
import { Dialog } from "./ui/Dialog";
import { Files } from "./ui/files/Files";
import { ModuleFileTable } from "./ui/files/ModuleFileTable";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "./ui/Tabs";
import { DetailsGallery } from "./Details/DetailsGallery";
import { DetailsMapsCard } from "./Details/DetailsMapsCard";
import { DetailsTable } from "./Details/DetailsTable";
import { DetailsView } from "./Details/DetailsView";

export const ModuleDetails = ({ path, id }: any) => {
  ...
  const result: any = objList.find((item: any) => item.name === headerPath);
  ...
  const formatLabel = (key: any) => {
    return (key && typeof key === "string") ? key.replace(/_/g, ' ').replace(/\b\w/g, (l: any) => l.toUpperCase()) : ""; // Capitalizes and removes underscores
  };
