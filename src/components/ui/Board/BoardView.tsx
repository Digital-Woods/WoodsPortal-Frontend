...
import { env } from "@/env";
import { getParam, toQueryString } from '@/utils/param';
...
export const BoardView = ({
  ...
  //Declaring
  const DragContext: any = createContext();

  // drag context component
  function Drag({ draggable = true, handleDrop, children }: any) {
    const [dragType, setDragType] = useState<any>(null); // if multiple types of drag item
    const [dragItem, setDragItem] = useState<any>(null); // the item being dragged
    const [isDragging, setIsDragging] = useState<any>(null);
    const [drop, setDrop] = useState<any>(null); // the active dropzone
    ...
    const dragStart = function (e: any, dragId: any, dragType: any) {
      e.stopPropagation();
    ...
    const drag = function (e: any, dragId: any, dragType: any) {
      e.stopPropagation();
      setIsDragging(true);
    };
  ...
  // a draggable item
  Drag.DragItem = function ({ as, dragId, dragType, ...props }: any) {
    const { draggable, dragStart, drag, dragEnd, dragItem }: any =
  ...
  Drag.DropZones = function ({
    ...
    const { dragType, isDragging }: any = useContext(DragContext);
    return (
      <div {...props}>
        {children}
  ...
  Drag.DropGuide = function ({ as, dropId, dropType, ...props }: any) {
    const { drop, dragType }: any = useContext(DragContext);
    let Component = as || "div";
  ...
  function Card({ dragItem, item, columns }: any) {
    ...
    function getSortedByBoardViewOrder(columns: any) {
      return columns
        .filter((col: any) => col.boardViewOrder !== null)
        .sort((a: any, b: any) => {
        ...
        {columns && getSortedByBoardViewOrder(columns).map((column: any, index: number) => (
          <div key={index}>
            {renderCellContent({
              ...
              associationPath:
                viewName === "ticket"
                  ? `/${item[column.key]}/${env.VITE_HUBSPOT_DEFAULT_OBJECT_IDS.tickets
  ...
  function List({ name, dragItem, children, count }: any) {
    return (
      <div
                ...
                <React.Fragment key={list.id}>
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {data[listPosition].cards.map((card: any, cardPosition: any) => {
                          return (
                            <Drag.DropZones
                              className="relative"
                              key={card.id}
