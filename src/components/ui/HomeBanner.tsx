...
const formatGreetingDate = (date = new Date()) => {
  ...
  return (formattedDate && typeof formattedDate === "string") ? formattedDate?.replace(/\d+/, `${day}${ordinalSuffix}`) : "";
};
...
export const HomeBanner = ({ moduleBannerDetailsOption, userData }: any) => {
  ...
  const { me } = useMe();
  // const loggedInDetails: any = useRecoilValue(userDetailsAtom);
  ...
  const filteredDetails = Object.entries(userDetails).filter(
    ([key, value]: any) => value?.label && !["hs_object_id"].includes(key)
  );
