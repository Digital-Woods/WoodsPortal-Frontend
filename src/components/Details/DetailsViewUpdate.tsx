...
const DetailsViewUpdateDialog = ({
  ...
  const { mutate: getStags, isLoading: sdfsf }: any = useMutation({
    mutationKey: ["getStageData1"],
    mutationFn: async (props) => {
      const {pipelineId, isNewValue, setValue }: any = props
      try {
        const response = await Client.details.stages({
          params: {
    ...
    onSuccess: async ({ response, pipelineId, isNewValue, setValue }: any) => {
      // let stagesM = stages;
  ...
  const onChangeSelect = (value: any, setValue: any) => {
